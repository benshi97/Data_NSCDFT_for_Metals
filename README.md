...
`quacc` requires python >= 3.11 and can be installed via pip. We recommend starting in a new python environment using [miniconda](https://docs.anaconda.com/miniconda/):
...
The scripts we provide must be added into the `PYTHONPATH` aftering installation of `quacc`:

```
git clone git@github.com:benshi97/Data_NSCDFT_for_Metals.git
